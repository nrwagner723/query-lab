...
def problem_one(request):
  ...
  students = Student.objects.all()
  greater_than_3 = Student.objects.filter(gpa__gt=3.0).order_by('-gpa')

  for student in greater_than_3:
...
def problem_two(request):
  ...
  hired_prior_2010 = Instructor.objects.filter(hire_date__lt=2010).order_by('hire_date')
  for instructor in hired_prior_2010:
...
def problem_three(request):
  ...
  pk_2 = Instructor.objects.get(pk=2)
  pk_2_courses = Course.objects.get(fk=2)
  print(f'''Full Name: {pk_2.first_name} {pk_2.last_name}
  Courses:''')
  for course in pk_2_courses:
    print(f' - {course.name}')
...
def problem_five(request):
  ...
  new_student = Student.objects.create(first_name='Natalie', last_name='Wagner', year=2022, gpa=3.7)
...
def problem_six(request):
  ...
  student_id = 11
  Student.objects.filter(pk=student_id).update(gpa=3.0)
  updated_new_student = Student.objects.get(pk=student_id)
  print(f'''ID: {updated_new_student.pk}
  Full name: {updated_new_student.first_name} {updated_new_student.last_name}
  GPA: {updated_new_student.gpa}''')
